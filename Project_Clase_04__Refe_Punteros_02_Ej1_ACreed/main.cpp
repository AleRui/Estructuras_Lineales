...
// Modificamos parra ser más eficiente
// los pasamos por referencia &.
// La funcion no modifica los argumentos
// Solo hace operacioens de lectura
// Por lo que pasamos refencias a objetos constantes
// hace que el código se más seguro
// auto same_name(Target const& t1, Target const& t2) -> bool // Predicado es que se devuelve verdadero o falso es BINARIO
// {
//     return t1.name == t2.name; // == lo podemos usar porue esta sobrecargado para comparar string
// }
...
// funcion que voy a convertir
// auto same_name(Target const& t1, Target const& t2) -> bool // Predicado es que se devuelve verdadero o falso es BINARIO
// {
//     return t1.name == t2.name; // == lo podemos usar porue esta sobrecargado para comparar string
// }
// (Target const& t1, Target const& t2) -> bool { return t1.name == t2.name; }

    //for (auto target_chunk : targets | stdv::chunk_by(same_name)) { // chunk_by agrupar por vista
    // Pasamos expresion lambda
    for (auto target_chunk : targets
                        // | stdv::chunk_by( [](Target const& t1, Target const& t2) -> bool { return t1.name == t2.name; })) {
                        // Y podemos quitar el tipo de retorno "-> bool" porque lo deduce el compilador
                        | stdv::chunk_by( [](Target const& t1, Target const& t2) { return t1.name == t2.name; })) {
                        // también podemos externalizar la función
         
        auto const name = stdr::begin(target_chunk)->name; // ve a la primera casilla del trozo del vector que has visto y coge lo que es
        // auto name = target_chunk.begin()->name; // Podría sustituir al anterior, pero más estricto para el tipo
        ...
        //     ++total;
        // }
        // Distance trabaja en un vector con casillas contiguas
        // target_chunk indicar donde empiezan y donde termian
        // el compilador calcula por diferencia de direcciones de memoria y divide entre el numero que hay
        // No es un contador
        // Tiene una complejidad unidad O(1). Siempre tarda lo mismo da igual el num de elementos en el vector
        auto const total = stdr::distance(target_chunk); // es funcion libre, más usado
        // auto total = target_chunk.size(); // es funcion propia menos usada Podría sustituir al anterior, pero más estricto para el tipo
